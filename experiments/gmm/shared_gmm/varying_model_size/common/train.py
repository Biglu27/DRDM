...
def create_simplified_metrics_dict(
    ...
    metrics_dict = {
        ...
        "train_loss": [
            {
                "trained": None,
                "trained_adjusted_memorized_means": None,
                "trained_only_memorized_means": None,
                "partial_mem_only_memorized_means": None,
            }
...
def train_model(
    ...
    num_components = training_config["num_components"]
    ...
    init_means_scale = training_config["init_means_scale"]
    init_var_scale = training_config["init_var_scale"]
    ...
    init_means, init_var, init_priors = iso_hom_gmm_create_initialization_parameters(
        key_init_model, init_strategy, dim, num_components, init_context_dict
    )

    model = IsoHomGMMSharedParametersEstimator(
        dim, num_components, vf_type, diffusion_process, init_means, init_var, init_priors
    )
    ...
    (
        ...
        key_loss_main_root,
        key_loss_adj_root,
        key_loss_only_mem_root,
        key_loss_pmem_only_mem_root,
    ) = jax.random.split(key_train, 6)
    ...
    sub_key_mem, sub_key_var, sub_key_means = jax.random.split(key_metrics, 3)
