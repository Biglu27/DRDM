...
def evaluate_baseline_vector_fields(
    baseline_config_file: Path, data_file: Path, output_dir: Path, use_wandb: bool = False
):
    """
    ...
    key = config["eval_key"]
    num_templates = config["num_templates"]  # Specific component count to evaluate
    ...
    for t_idx in range(num_times):
        ...
        key_t_gt_train, key_t_gt_val, key_t_mem_train, key_t_mem_val = jax.random.split(
            key_t_gt_mem_fold, 4
        )
        ...
        (
            ...
            key_pmem_loss_train,
            key_pmem_loss_val,
            key_pmem_gtm_choice_samples,
            key_pmem_gtm_choice_means,
            key_pmem_gtm_loss_train,
            key_pmem_gtm_loss_val,
        ) = jax.random.split(key_iter_root, 7)

        # Partial Memorization (using subset of training samples)
        if num_templates < num_samples_train:
            samples_to_memorize_indices = jax.random.choice(
                key_pmem_choice, num_samples_train, (num_templates,), replace=False
            )
        else:
...
def main():
    parser = argparse.ArgumentParser(
        description="Evaluate baseline vector fields for a specific component count."
    )
    parser.add_argument(
        "--baseline-config-file",
        type=str,
        required=True,
        help="Path to baseline config pickle file",
    )
    ...
    evaluate_baseline_vector_fields(
        Path(args.baseline_config_file), Path(args.data_file), Path(args.output_dir), args.use_wandb
    )
